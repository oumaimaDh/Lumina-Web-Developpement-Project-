...
class OfferController {
    ...
    // Récupérer toutes les offres (only for job department associations - categories 5-9)
    public function getOffers() {
        $conn = Config::getConnexion();
        $sql = "SELECT o.*, a.name as association_name 
                FROM offers o 
                LEFT JOIN association a ON o.id_association = a.id_association 
    ...
    public function addOffer($offerData) {
        $conn = Config::getConnexion();
        $sql = "INSERT INTO offers (id_association, title, location, salary_min, salary_max, expiration_date, description, contract_types, skills, status) 
                VALUES (:id_association, :title, :location, :salary_min, :salary_max, :expiration_date, :description, :contract_types, :skills, :status)";

        try {
            $query = $conn->prepare($sql);
            $query->execute([
                ':id_association' => $offerData['association_id'] ?? $offerData['id_association'],
