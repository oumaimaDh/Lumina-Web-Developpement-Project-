...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'submit_application') {
    ...
    require_once $basePath . DIRECTORY_SEPARATOR . 'controller' . DIRECTORY_SEPARATOR . 'OfferController.php';
    require_once $basePath . DIRECTORY_SEPARATOR . 'controller' . DIRECTORY_SEPARATOR . 'AssociationController.php';
    require_once $basePath . DIRECTORY_SEPARATOR . 'controller' . DIRECTORY_SEPARATOR . 'ApplicationController.php';
...
require_once $_SERVER['DOCUMENT_ROOT'] . '/ProjetMVC/controller/ApplicationController.php';
...
$offers = array_filter($allOffers, function($offer) use ($associationId) {
    ...
    $offerAssociationId = $offer['id_association'] ?? $offer['association_id'] ?? null;
    return $offerAssociationId && (int)$offerAssociationId == (int)$associationId;
});
...
function extractRating($ratingText) {
    if ($ratingText !== null && $ratingText !== '' && preg_match('/(\d+\.\d+)/', $ratingText, $matches)) {
...
foreach ($offers as $offer) {
    $offersData[] = [
        'id' => $offer['id'] ?? $offer['id_offer'] ?? null,
        'title' => $offer['title'] ?? '',
        'location' => $offer['location'] ?? '',
        'salary_min' => $offer['salary_min'] ?? 0,
        'salary_max' => $offer['salary_max'] ?? 0,
                ...
                <div class="location-details-header">
                    <div class="location-info-header">
                        ...
                        <div class="header-contact-actions">
                            <a href="#" class="header-contact-btn primary" id="directions-btn" target="_blank">
                    ...
                    <div class="offer-card">
                        <div class="offer-header">
                            <div class="offer-main">
                                ...
                                <div class="offer-salary"><?php echo $offer['salary_min'] ?? 0; ?>-<?php echo $offer['salary_max'] ?? 0; ?> DT/month</div>
                            </div>
...
<div class="modal-overlay" id="application-modal">
   ...
   <script>
    ...
    const associationLocation = <?php echo json_encode($association['location'] ?? ''); ?>;
    ...
    // ========== MAP FUNCTIONS ==========
    // Initialize map in header with dynamic geocoding
    async function initMap() {
        // Initialize map with Tunisia center
        const map = initTunisiaMap('association-map', 33.8869, 10.1775, 7);
        
        let locationCoords = null;
        let locationAddress = associationLocation || 'Location not available';
        
        // Try to get coordinates from association location
        if (associationLocation && associationLocation.trim() !== '') {
            // First try to parse as coordinates
            locationCoords = parseCoordinates(associationLocation);
            
            // If not coordinates, geocode the address
            if (!locationCoords) {
                locationCoords = await getLocationCoordinates(associationLocation);
            }
            
            if (locationCoords) {
                // Use geocoded address if available
                if (locationCoords.display_name) {
                    locationAddress = locationCoords.display_name;
                }
                
                // Center map on association location
                map.setView([locationCoords.lat, locationCoords.lng], 15);
                
                // Add custom marker icon for association
                ...
                // Add marker with custom icon - store reference for later use
                const associationMarker = L.marker([locationCoords.lat, locationCoords.lng], { icon: customIcon }).addTo(map);
                
                // Add popup
                associationMarker.bindPopup(`
                ...
                // Store coordinates globally for directions button
                window.associationCoords = locationCoords;
                
                // Update location details
                updateLocationDetails(locationCoords, locationAddress);
            } else {
                // If geocoding failed, show default location
                const defaultCoords = { lat: 33.8869, lng: 10.1775 };
                updateLocationDetails(defaultCoords, locationAddress);
            }
        } else {
            // No location available
            updateLocationDetails({ lat: 33.8869, lng: 10.1775 }, 'Location not available');
        }
    }

    // Update location information in header
    function updateLocationDetails(locationCoords, locationAddress) {
        const addressParts = locationAddress.split(',');
        document.getElementById('location-address').textContent = addressParts[0] || locationAddress;
        
        // Set default business hours and contact info
        document.getElementById('business-hours').textContent = 'Mon-Fri: 8AM-6PM';
        document.getElementById('contact-info').textContent = '+216 XX XXX XXX';
        document.getElementById('transportation').textContent = 'Available';
        
        // Update directions button
        const directionsBtn = document.getElementById('directions-btn');
        if (locationCoords && locationCoords.lat && locationCoords.lng) {
            const directionsUrl = `https://www.google.com/maps/dir/?api=1&destination=${locationCoords.lat},${locationCoords.lng}`;
