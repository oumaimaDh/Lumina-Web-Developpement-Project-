...
// Récupérer les catégories 5-9 depuis la base de données pour job department
$db = Config::getConnexion();
$sql = "SELECT id_category, name FROM category WHERE id_category IN (5, 6, 7, 8, 9) ORDER BY id_category";
try {
    $query = $db->prepare($sql);
    $query->execute();
    $jobCategories = $query->fetchAll(PDO::FETCH_ASSOC);
} catch(Exception $e) {
    $jobCategories = [];
}

// Map category IDs to category keys for controller
$categoryIdToKey = [
    5 => 'health',
    6 => 'education',
    7 => 'commerce',
...
// Map category keys to icons and display names
$categoryKeyToIcon = [
    'health' => '🏥',
    'education' => '🎓',
    'commerce' => '🛍️',
...
function getLogoByCategory($category) {
    $logos = [
        ...
        'restaurant' => 'https://images.unsplash.com/photo-1554118811-1e0d58224f24?w=80&h=80&fit=crop&crop=center',
        'restaurants' => 'https://images.unsplash.com/photo-1554118811-1e0d58224f24?w=80&h=80&fit=crop&crop=center', // Legacy support
...
function getBannerByCategory($category) {
    $banners = [
        ...
        'restaurant' => 'https://images.unsplash.com/photo-1559925393-8be0ec4767c8?w=400&h=120&fit=crop&crop=center',
        'restaurants' => 'https://images.unsplash.com/photo-1559925393-8be0ec4767c8?w=400&h=120&fit=crop&crop=center', // Legacy support
...
function getSectorByCategory($category) {
    $sectors = [
        ...
        'restaurants' => 'Hospitality & Food Services', // Legacy support
...
function getTypeByCategory($category) {
    $types = [
        ...
        'restaurants' => 'Café & Restaurant', // Legacy support
...
?>
      ...
      <section class="categories-section" id="job-search">
          <div class="container">
              ...
              <div class="categories-filter animated" id="categories-filter">
                  <button class="category-btn active" data-category="all" data-category-id="all">All</button>
                  <?php foreach ($jobCategories as $cat): 
                      $categoryKey = $categoryIdToKey[$cat['id_category']] ?? 'other';
                      $icon = $categoryKeyToIcon[$categoryKey] ?? '💼';
                  ?>
                      <button class="category-btn" data-category="<?php echo htmlspecialchars($categoryKey); ?>" data-category-id="<?php echo htmlspecialchars($cat['id_category']); ?>">
                          <?php echo $icon . ' ' . htmlspecialchars($cat['name']); ?>
                      </button>
                  <?php endforeach; ?>
              </div>
          </div>
  ...
  <script>
      ...
      // Pass category mapping from PHP to JavaScript
      const categoryIdToKey = <?php echo json_encode($categoryIdToKey); ?>;
      const categoryKeyToId = {};
      Object.keys(categoryIdToKey).forEach(id => {
          categoryKeyToId[categoryIdToKey[id]] = parseInt(id);
      });
