...
require_once $_SERVER['DOCUMENT_ROOT'] . '/ProjetMVC/controller/socialcasecontroller.php';
        ...
        $filteredAssociations = array_filter($associations, function($association) {
            return $association['id_category'] == 3;
        });
...
Design:<a href="https://templatemo.com" target="_blank">Lumina Team</a></p>
  ...
  <script>
    ...
    document.addEventListener('DOMContentLoaded', async function() {
      ...
      for (let i = 0; i < associations.length; i++) {
        ...
        try {
          ...
          if (coords && !isNaN(coords.lat) && !isNaN(coords.lng)) {
            ...
            markers.push(marker);
            processed++;
            console.log(`✓ Added marker for: ${assoc.name}`);
          } else {
      ...
      console.log(`Summary: ${markers.length}/${associations.length} markers added`);
      ...
      if (markers.length > 0) {
        ...
        loadingDiv.innerHTML = `
          <div style="color: #f44336;">
            ...
            <small>Failed: ${failed.length} associations</small>
          </div>
        `;
